...
export function AdminStaffPage() {
  ...
  const assignMutation = useMutation({
    mutationFn: ({ scheduleId, staffId }: any) => schedulesApi.update(scheduleId, { staff_id: staffId }),
    onSuccess: () => {
  ...
  const handleAssignSubmit = () => {
    ...
    assignMutation.mutate({ scheduleId: selectedSchedule, staffId: assigningStaff.staff_id });
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid gap-4">
        {(staff?.data || staff)?.map((s: any) => (
          <div key={s.staff_id} className="bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <h3 className="text-xl font-semibold text-gray-900">Staff #{s.staff_id}</h3>
                <p className="text-gray-600">User ID: {s.user_id}</p>
                <div className="mt-2">
                  <p className="text-sm text-gray-600">{s.staff_bio}</p>
                </div>
                {s.staff_special && (
                  <div className="mt-2">
                    <span className="text-sm text-gray-500">Special Skills: </span>
                    <span className="text-sm text-gray-700">{s.staff_special}</span>
                  </div>
                )}
              </div>
                ...
                <button
                  onClick={() => {
                    if (confirm('Delete this staff member?')) {
                      deleteMutation.mutate(s.staff_id);
                    }
                  }}
                ...
                >
                  ...
                  {(schedules?.data || schedules)?.map((schedule: any) => (
                    <option key={schedule.schedule_id} value={schedule.schedule_id}>
                      Schedule #{schedule.schedule_id} - {new Date(schedule.start_time).toLocaleString()}
                      {schedule.staff_id ? ` (Currently: Staff #${schedule.staff_id})` : ' (Unassigned)'}
                    </option>
                  ))}
                </select>
              </div>
