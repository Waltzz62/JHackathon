...
export function AdminSchedulesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setIsCreating(false);
      setFormData({ class_id: '', staff_id: '', start_time: '', end_time: '' });
    },
  });
              ...
              <div>
                ...
                >
                  ...
                  {(classes?.data || classes)?.map((cls: any) => (
                    <option key={cls.class_id} value={cls.class_id}>{cls.class_title}</option>
                  ))}
                </select>
              </div>
              <div>
                ...
                >
                  ...
                  {(staff?.data || staff)?.map((s: any) => (
                    <option key={s.staff_id} value={s.staff_id}>Staff #{s.staff_id}</option>
                  ))}
                </select>
              </div>
              <div>
          ...
          <div key={schedule.schedule_id} className="bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <h3 className="text-xl font-semibold text-gray-900">Schedule #{schedule.schedule_id}</h3>
